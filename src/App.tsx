...
// 修正：在此處引入 getDocs 以便抓取並刪除舊有投票資料
import { getFirestore, collection, onSnapshot, doc, setDoc, updateDoc, deleteDoc, increment, arrayUnion, arrayRemove, getDoc, getDocs } from 'firebase/firestore';
                ...
                {teacherLayout === 'desktop' ? (
                  <React.Fragment>
                    ...
                    <div className="lg:col-span-4 flex flex-col gap-6">
                      <div className={`shrink-0 flex flex-col p-5 rounded-2xl shadow-sm border-2 relative overflow-hidden transition-colors ${(activePollId && currentActivePollData) ? (currentActivePollData.type === 'vote' ? 'bg-white border-purple-100' : currentActivePollData.type === 'quiz' ? 'bg-white border-emerald-100' : 'bg-white border-rose-100') : (currentTopic ? 'bg-white border-indigo-100' : 'bg-slate-50 border-slate-200')}`}>
                        ...
                        {activePollId && currentActivePollData ? (
                          <div className="animate-in fade-in flex flex-col flex-1">
                            ...
                            {/* 墊高區塊將按鈕推至底部，避免 flex h-full 導致切斷 */}
                            ...
                            {pollState === 'voting' ? (
                              <button onClick={handleRevealPoll} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 rounded-xl text-sm transition-colors flex items-center justify-center gap-2 shadow-md">
                          ...
                          <div className="flex-1 flex items-center justify-center text-slate-400 font-bold min-h-[6rem] bg-slate-50/50 rounded-lg">
                            （目前無指定主題，學生自由留言）
                          </div>
                        )}
                      </div>
